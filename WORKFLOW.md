...
3. Система собирает **ВСЕ** параметры:
   ...
   - ✅ **База данных** (`baseContent`) - если загружена (ИЗОЛИРОВАНА по вкладкам!)
...
Примечание: Язык ответа определяется из контекста промпта и языка вопроса пользователя.`
    },
    {
      role: 'user',
      content: inputText
...
**Каждая вкладка хранит ОТДЕЛЬНО (полная изоляция):**
...
- `gptModel` - модель GPT
- `baseContent` - содержимое базы данных (ИЗОЛИРОВАНО!)
- `baseFileName` - имя файла базы данных (ИЗОЛИРОВАНО!)
- История ответов для этой вкладки

**ОБЩИЕ для всех вкладок:**
- `apiKey` - API ключ OpenAI
- `tabs` - метаданные вкладок (ID, имя, дата создания)
- `currentTabId` - ID активной вкладки

**КРИТИЧЕСКИ ВАЖНО - ДВОЙНОЕ СОХРАНЕНИЕ:**

Промпты, настройки и база данных должны сохраняться В ДВА МЕСТА:
1. В `tab_{id}_data` - для системы вкладок popup (изолированное хранилище каждой вкладки)
2. В глобальные ключи - для content.js (плавающая кнопка читает из них)

**Что сохраняется в оба места:**
- `positivePrompt` - положительный промпт
- `negativePrompt` - отрицательный промпт
- `gptModel` - модель GPT
- `baseContent` - база данных (НОВОЕ: теперь изолируется по вкладкам!)
- `baseFileName` - имя файла базы (НОВОЕ: теперь изолируется по вкладкам!)

**Почему это важно:**
- Popup работает с системой вкладок (`tab_{id}_data`)
- Content.js (плавающая кнопка на странице) читает глобальные ключи
- Без синхронизации content.js не увидит промпты/базу из popup
- При переключении вкладок глобальные ключи обновляются данными активной вкладки

**Когда синхронизировать:**
- При изменении промпта/настройки (событие `change`)
- При загрузке файла базы данных (событие `change` на input[type=file])
- При загрузке вкладки (`loadTabData()`) - ВАЖНО: синхронизирует данные активной вкладки в глобальные ключи
...
- При изменении промптов → сохранить промпты (двойное сохранение)
- При изменении модели → сохранить модель (двойное сохранение)
- При загрузке файла базы → сохранить `baseContent` и `baseFileName` (двойное сохранение)
- После генерации ответа → сохранить `answerText`
- При редактировании названия вкладки → сохранить название
- При переключении вкладок → синхронизировать глобальные ключи с активной вкладкой
